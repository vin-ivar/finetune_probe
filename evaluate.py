...
def main():
    ...
    config = Params.from_file(args.config, ext_vars={'model_name': model_name, 'model_size': "768",
                                                     'train_path': "", 'val_path': "", 'freeze': ""})
    cuda_device = config.get('trainer').get('cuda_device')

    reader = DatasetReader.from_params(config.get('dataset_reader'))
    model = Model.load(config.duplicate(), args.path, os.path.join(args.path, 'best.th'), cuda_device=cuda_device)
